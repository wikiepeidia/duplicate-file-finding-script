            ...
            if ($sizeGroup.Count -gt 1) {
                ...
                foreach ($hashValue in $contentHashes.Keys) {
                    ...
                    foreach ($firstFile in $duplicateFiles) {
                        $processedKey = $firstFile.FullName

                        if (-not $processedFiles.ContainsKey($processedKey)) {
                            foreach ($duplicateFile in $duplicateFiles) {
                                if ($firstFile -ne $duplicateFile) {
                                    $duplicateGroup += $duplicateFile.FullName
                                    $processedFiles[$processedKey] = $true
                                }
                            }

                            Write-Host "$($firstFile.FullName) is a duplicate of $($duplicateGroup -join ' and ')"
